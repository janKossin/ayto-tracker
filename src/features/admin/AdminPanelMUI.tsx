...
import { ProbabilityCacheService } from "@/services/probabilityCacheService";
...
const ParticipantForm: React.FC<{
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Dies sollte idealerweise im Backend passieren oder wir lassen es hier weg, da API Calls async sind
      // Backend handles cascading updates usually via foreign keys or manual logic
      // In Prisma schema names are just strings, so we might lose references if we don't update them everywhere.
      // But for now let's focus on basic CRUD.
  ...
  return (
    <Card>
      <CardHeader
        title={
          initial ? "Kandidat*in bearbeiten" : "Neue*n Kandidat*in hinzufügen"
        }
        avatar={
          ...
          <Stack spacing={3}>
            ...
            >
              ...
              <TextField
                ...
                onChange={(e) =>
                  setForm({ ...form, knownFrom: e.target.value })
                }
              />
              <TextField
                fullWidth
              ...
              <TextField
                ...
                onChange={(e) =>
                  setForm({ ...form, socialMediaAccount: e.target.value })
                }
            ...
            <FormControl component="fieldset">
              ...
              <RadioGroup
                ...
                value={form.gender}
                onChange={(e) =>
                  setForm({ ...form, gender: e.target.value as "F" | "M" })
                }
              >
                    ...
                    backgroundImage:
                      participant.photoUrl && participant.photoUrl.trim() !== ""
                        ...
                        : participant.gender === "F"
                          ? "linear-gradient(135deg, #E0B05F 0%, #A6752A 100%)"
                          : "linear-gradient(135deg, #E03A44 0%, #8A080F 100%)",
...
const MatchboxManagement: React.FC<{
  ...
  const [editingMatchbox, setEditingMatchbox] = useState<Matchbox | undefined>(
    undefined,
  );
  const [matchboxForm, setMatchboxForm] = useState<
    Omit<Matchbox, "id" | "createdAt" | "updatedAt">
  >({
    woman: "",
    man: "",
    matchType: "no-match",
  ...
  const availableWomen = editingMatchbox
    ? women
    : women.filter(
        (woman) =>
          !matchboxes.some(
            (mb) => mb.matchType === "perfect" && mb.woman === woman.name,
          ),
      );
  const availableMen = editingMatchbox
    ? men
    : men.filter(
        (man) =>
          !matchboxes.some(
            (mb) => mb.matchType === "perfect" && mb.man === man.name,
          ),
      );

  const perfectMatches = matchboxes.filter(
    (mb) => mb.matchType === "perfect",
  ).length;
  const noMatches = matchboxes.filter(
    (mb) => mb.matchType === "no-match",
  ).length;
  const totalRevenue = matchboxes
    .filter(
      (mb) =>
        mb.matchType === "sold" && mb.price && typeof mb.price === "number",
    )
  ...
  const saveMatchbox = async () => {
    try {
      if (!matchboxForm.woman || !matchboxForm.man) {
        setSnackbar({
          open: true,
          message: "Bitte wähle eine Frau und einen Mann aus!",
          severity: "error",
        });
        return;
      }
      ...
      if (matchboxForm.matchType === "sold") {
        if (!matchboxForm.price || matchboxForm.price <= 0) {
          setSnackbar({
            open: true,
            message:
              "Bei verkauften Matchboxes muss ein gültiger Preis angegeben werden!",
            severity: "error",
          });
          return;
        }
        if (!matchboxForm.buyer) {
          setSnackbar({
            open: true,
            message:
              "Bei verkauften Matchboxes muss ein Käufer ausgewählt werden!",
            severity: "error",
          });
          return;
        }
      ...
      if (editingMatchbox) {
        // Verwende Service für das Aktualisieren

        await MatchboxService.updateMatchbox(editingMatchbox.id!, {
          ...matchboxForm,
        });

        setSnackbar({
          open: true,
          message: "Matchbox wurde erfolgreich aktualisiert!",
          severity: "success",
        });
      } else {
        ...
        setSnackbar({
          open: true,
          message: "Matchbox wurde erfolgreich erstellt!",
          severity: "success",
        });
      }
  ...
  const deleteMatchbox = async (id: number) => {
    if (confirm("Sind Sie sicher, dass Sie diese Matchbox löschen möchten?")) {
      try {
        ...
        setSnackbar({
          open: true,
          message: "Matchbox erfolgreich gelöscht",
          severity: "success",
        });
      } catch (error) {
        ...
        <CardContent>
          {matchboxes.length === 0 ? (
            ...
            >
              {matchboxes
                .sort((a, b) => {
                  const dateA = a.ausstrahlungsdatum
                    ? new Date(a.ausstrahlungsdatum).getTime()
                    : 0;
                  const dateB = b.ausstrahlungsdatum
                    ? new Date(b.ausstrahlungsdatum).getTime()
                    : 0;
                  return dateB - dateA;
                })
                .map((matchbox) => (
                      ...
                      {matchbox.ausstrahlungsdatum && (
                        <Typography variant="caption" color="text.secondary">
                          ...
                          {matchbox.ausstrahlungszeit &&
                            ` um ${matchbox.ausstrahlungszeit} Uhr`}
                        </Typography>
                      )}
                    </CardContent>
                  </Card>
        ...
        <DialogContent>
          <Stack spacing={3} sx={{ mt: 2 }}>
            ...
            >
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  label="Frau"
                  onChange={(e) =>
                    setMatchboxForm({ ...matchboxForm, woman: e.target.value })
                  }
                >
              ...
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  label="Mann"
                  onChange={(e) =>
                    setMatchboxForm({ ...matchboxForm, man: e.target.value })
                  }
                >
            ...
            <Alert severity="info" icon={<ScheduleIcon />}>
              Die Ausstrahlungszeiten werden zentral über den{" "}
              <strong>Ausstrahlungsplan</strong> verwaltet.
            </Alert>
              ...
              >
                <TextField
                  ...
                  onChange={(e) => {
                    ...
                    setMatchboxForm({
                      ...matchboxForm,
                      price: isNaN(value) ? undefined : value,
                    });
                  }}
                  placeholder="0.00"
                />
...
const MatchingNightManagement: React.FC<{
  ...
  const [editingMatchingNight, setEditingMatchingNight] = useState<
    MatchingNight | undefined
  >(undefined);
  ...
  const manualLights = Math.max(
    0,
    matchingNightForm.totalLights - totalPerfectMatches,
  );
  ...
  const addPair = () => {
    if (selectedWoman && selectedMan) {
      setMatchingNightForm({
        ...matchingNightForm,
        pairs: [
          ...matchingNightForm.pairs,
          { woman: selectedWoman, man: selectedMan },
        ],
      });
      setSelectedWoman("");
      setSelectedMan("");
  ...
  const saveMatchingNight = async () => {
    try {
      ...
      if (matchingNightForm.totalLights > 10) {
        setSnackbar({
          open: true,
          message: "Maximum 10 Lichter erlaubt!",
          severity: "error",
        });
        return;
      }
      ...
      const completePairs = matchingNightForm.pairs.filter(
        (pair) => pair && pair.woman && pair.man,
      );
      ...
      const genderConflicts = completePairs.filter((pair) => {
        const womanParticipant = participants.find(
          (p) => p.name === pair.woman,
        );
        ...
        return (
          womanParticipant &&
          manParticipant &&
          womanParticipant.gender === manParticipant.gender
        );
      });
      ...
      const validPerfectMatches = getValidPerfectMatchesForMatchingNight(
        matchboxes,
        tempMatchingNight,
      );
      const perfectMatchLights = completePairs.filter((pair) =>
        validPerfectMatches.some(
      ...
      if (editingMatchingNight) {
        ...
        await MatchingNightService.updateMatchingNight(
          editingMatchingNight.id!,
          {
        ...
        setSnackbar({
          open: true,
          message: "Matching Night wurde erfolgreich aktualisiert!",
          severity: "success",
        });
      } else {
        ...
        setSnackbar({
          open: true,
          message: `Matching Night "${autoGeneratedName}" wurde erfolgreich erstellt!`,
          severity: "success",
        });
      }
  ...
  const deleteMatchingNight = async (id: number) => {
    if (confirm("Sind Sie sicher, dass Sie diesen Eintrag löschen möchten?")) {
      try {
        // Assuming 'activeTab' is available in this scope or passed as an argument
        // For this specific file, we are only dealing with Matching Nights,
        // so we can directly call MatchingNightService.deleteMatchingNight.
        // If this handleDelete function is meant to be generic across different tabs,
        // then 'activeTab' would need to be defined and managed.
        // For now, I'll assume it's specific to Matching Nights context or activeTab=1.
        await MatchingNightService.deleteMatchingNight(id);

        // Assuming loadAllData and onUpdate are equivalent or loadAllData covers onUpdate's functionality
        onUpdate(); // Or loadAllData() if it's available and appropriate here
        setSnackbar({
          open: true,
                      ...
                      >
                        ...
                        <Box>
                          ...
                          <Tooltip title="Löschen">
                            <IconButton
                              onClick={() =>
                                deleteMatchingNight(matchingNight.id!)
                              }
                              color="error"
                            >
                              <DeleteIcon />
                      ...
                      {matchingNight.ausstrahlungsdatum && (
                        <Typography variant="body2" color="text.secondary">
                          ...
                          {matchingNight.ausstrahlungszeit &&
                            ` um ${matchingNight.ausstrahlungszeit} Uhr`}
                        </Typography>
                      )}
                    </CardContent>
                  </Card>
            ...
            >
              ...
              <TextField
                ...
                onChange={(e) =>
                  setMatchingNightForm({
                    ...matchingNightForm,
                    totalLights: parseInt(e.target.value) || 0,
                  })
                }
                placeholder="0"
              />
                ...
                <CardContent>
                    ...
                    <Card variant="outlined">
                      <CardContent sx={{ textAlign: "center", py: 2 }}>
                        <Typography variant="h5" color="text.secondary">
                          {Math.max(0, 10 - matchingNightForm.pairs.length)}
                        </Typography>
                        <Typography variant="caption">
                          Fehlende Paare
                        </Typography>
                  ...
                  {totalPerfectMatches > matchingNightForm.totalLights && (
                    <Alert severity="warning" sx={{ mt: 2 }}>
                      Achtung: Mehr bekannte Perfect Matches (
                      {totalPerfectMatches}) als Gesamtlichter (
                      {matchingNightForm.totalLights})!
                    </Alert>
                  )}
                </CardContent>
...
const SettingsManagement: React.FC<{
  ...
  const [editingPenalty, setEditingPenalty] = useState<Penalty | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    ...
    if (savedBudget) {
      setBudgetSettings((prev) => ({
        ...prev,
        startingBudget: parseInt(savedBudget, 10),
      }));
    }
  }, []);
  ...
  const saveBudgetSettings = () => {
    ...
    if (isNaN(budget) || budget < 0) {
      setSnackbar({
        open: true,
        message: "❌ Bitte geben Sie eine gültige Summe ein!",
        severity: "error",
      });
      return;
    }
    ...
    setSnackbar({
      open: true,
      message: `✅ Startsumme wurde auf ${budget.toLocaleString("de-DE")} € gesetzt!`,
      severity: "success",
    });
  };
    ...
    try {
      ...
      if (
        ...
        setSnackbar({
          open: true,
          message:
            "❌ Bitte füllen Sie alle Pflichtfelder aus und geben Sie einen Betrag ≠ 0 ein!",
          severity: "error",
        });
        return;
      }
      ...
      if (editingPenalty) {
        ...
        setSnackbar({
          open: true,
          message: "✅ Transaktion wurde erfolgreich aktualisiert!",
          severity: "success",
        });
      } else {
        // Verwende Service für das Hinzufügen
        ...
        setSnackbar({
          open: true,
          message: "✅ Transaktion wurde erfolgreich hinzugefügt!",
          severity: "success",
        });
      }
  ...
  const deletePenalty = async (id: number) => {
    setConfirmDialog({
      ...
      message:
        "Wirklich diese Strafe löschen?\n\nDieser Vorgang kann nicht rückgängig gemacht werden!",
      severity: "warning",
      onConfirm: async () => {
        try {
          // Verwende Service für das Löschen
          await PenaltyService.deletePenalty(id);

          await onUpdate();
          setSnackbar({
            open: true,
            message: "Transaktion wurde erfolgreich gelöscht!",
            severity: "success",
          });
        } catch (error) {
  ...
  const exportParticipants = async () => {
    try {
      ...
      setSnackbar({
        open: true,
        message: `✅ ${data.length} Kandidat*innen wurden exportiert!`,
        severity: "success",
      });
    } catch (error) {
      ...
      setSnackbar({
        open: true,
        message: `❌ Fehler beim Export: ${error instanceof Error ? error.message : "Unbekannter Fehler"}`,
        severity: "error",
      });
    }
  };
  ...
  const exportMatchingNights = async () => {
    try {
      ...
      setSnackbar({
        open: true,
        message: `✅ ${data.length} Matching Nights wurden exportiert!`,
        severity: "success",
      });
    } catch (error) {
  ...
  const exportMatchboxes = async () => {
    try {
      ...
      setSnackbar({
        open: true,
        message: `✅ ${data.length} Matchboxes wurden exportiert!`,
        severity: "success",
      });
    } catch (error) {
  ...
  const exportPenalties = async () => {
    try {
      ...
      setSnackbar({
        open: true,
        message: `✅ ${data.length} Strafen/Transaktionen wurden exportiert!`,
        severity: "success",
      });
    } catch (error) {
  ...
  const exportAllData = async () => {
    try {
      const [
        ...
        ProbabilityCacheService.getAllProbabilityCache(),
      ...
      setSnackbar({
        open: true,
        message: `❌ Fehler beim kompletten Export: ${error instanceof Error ? error.message : "Unbekannter Fehler"}`,
        severity: "error",
      });
    }
  };
  ...
  const exportForDeploy = async () => {
    try {
      ...
      const invalidMN = matchingNightsData.filter(
        (m) => !m.ausstrahlungsdatum || !m.ausstrahlungszeit,
      );
      const invalidMB = matchboxesData.filter(
        (m) => !m.ausstrahlungsdatum || !m.ausstrahlungszeit,
      );
  ...
  const updateIndexJsonForDeploy = async (fileName: string) => {
    try {
      ...
      if (!currentFiles.includes(fileName)) {
        ...
        console.log(
          `📝 Index.json würde aktualisiert werden mit:`,
          currentFiles,
        );
        console.log(
          `ℹ️ Für das Deployment muss diese Datei manuell in public/json/index.json gespeichert werden`,
        );
      }
    } catch (error) {
      ...
      onConfirm: async () => {
        try {
          ...
          await Promise.all(
            all.map((p) =>
              p.id
                ? ParticipantService.deleteParticipant(p.id)
                : Promise.resolve(),
            ),
          );
          await onUpdate();
          setSnackbar({
            open: true,
            message: "Alle Kandidat*innen wurden erfolgreich gelöscht!",
            severity: "success",
          });
        } catch (error) {
  ...
  const deleteMatchingNights = async () => {
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          setIsLoading(true);
          const all = await MatchingNightService.getAllMatchingNights();
          await Promise.all(
            all.map((n) =>
              n.id
                ? MatchingNightService.deleteMatchingNight(n.id)
                : Promise.resolve(),
            ),
          );
          await onUpdate();
          setSnackbar({
            open: true,
            message: "Alle Matching Nights wurden erfolgreich gelöscht!",
            severity: "success",
          });
        } catch (error) {
  ...
  const deleteMatchboxes = async () => {
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          setIsLoading(true);
          const all = await MatchboxService.getAllMatchboxes();
          await Promise.all(
            all.map((m) =>
              m.id ? MatchboxService.deleteMatchbox(m.id) : Promise.resolve(),
            ),
          );
          await onUpdate();
          setSnackbar({
            open: true,
            message: "Alle Matchboxes wurden erfolgreich gelöscht!",
            severity: "success",
          });
        } catch (error) {
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          ...
          const menNames = allParticipants
            .filter((p) => p.gender === "M")
            .map((p) => p.name);
          const womenNames = allParticipants
            .filter((p) => p.gender === "F")
            .map((p) => p.name);

          console.log("👥 Kandidat*innen:", {
            männer: menNames.length,
            frauen: womenNames.length,
          });
          ...
          for (const night of allMatchingNights) {
            ...
            const menNamesInMenField = Array.from(menInNight).filter((name) =>
              menNames.includes(name),
            );
            const womenNamesInMenField = Array.from(menInNight).filter((name) =>
              womenNames.includes(name),
            );

            const isSwapped =
              womenNamesInMenField.length > menNamesInMenField.length;

            if (isSwapped) {
              console.warn(
                `⚠️ Matching Night "${night.name}" (ID: ${night.id}): Paare sind vertauscht!`,
              );
          ...
          console.log(
            `✅ Korrektur abgeschlossen: ${correctedCount} Matching Night(s) korrigiert`,
          );

          // Zeige Erfolgsmeldung
          if (correctedCount > 0) {
            alert(
              `✅ ${correctedCount} Matching Night(s) erfolgreich korrigiert!\n\nDie Seite wird neu geladen.`,
            );
            // Seite neu laden, um alle Daten zu aktualisieren
            window.location.reload();
          } else {
            alert(
              "✅ Keine fehlerhaften Matching Nights gefunden - alles korrekt!",
            );
            setIsLoading(false);
          }
        } catch (error) {
          ...
          alert(
            `❌ Fehler bei der Korrektur: ${error instanceof Error ? error.message : "Unbekannter Fehler"}`,
          );
          setIsLoading(false);
        }
      },
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          ...
          console.log(
            `✅ Zeitstempel-Korrektur abgeschlossen: ${nightsUpdated} Matching Nights und ${boxesUpdated} Matchboxes aktualisiert`,
          );

          alert(
            `✅ Zeitstempel erfolgreich korrigiert!\n\n${boxesUpdated} Matchboxes (20:15)\n${nightsUpdated} Matching Nights (21:00)\n\nDie Seite wird neu geladen.`,
          );
          window.location.reload();
        } catch (error) {
      ...
      onConfirm: async () => {
        try {
          ...
          setSnackbar({
            open: true,
            message:
              "✅ Browser wurde komplett zurückgesetzt! Cache, Cookies und alle Daten wurden gelöscht. Die Seite wird neu geladen.",
            severity: "success",
          });
          ...
          setSnackbar({
            open: true,
            message: `❌ Fehler beim Browser-Reset: ${error instanceof Error ? error.message : "Unbekannter Fehler"}`,
            severity: "error",
          });
        } finally {
          setIsLoading(false);
...
Alle Daten gehen unwiderruflich verloren!`);
    ...
    try {
      ...
      // Alle Tabellen leeren
      // Reset via API import logic (clearing everything)
      await ApiClient.post("/import", { clearBeforeImport: true });
      ...
      setSnackbar({
        open: true,
        message:
          "✅ Datenbank wurde komplett zurückgesetzt! Alle Daten wurden erfolgreich gelöscht.",
        severity: "success",
      });
  ...
  // ** Import Functions **
  const importParticipantsJSON = async (
    ...
    try {
      ...
      const normalizedParticipants = arr.map((participant: any) => {
        ...
        return {
          ...
          age: participant.age
            ? parseInt(participant.age.toString(), 10)
            : undefined,
      ...
      setConfirmDialog({
        ...
        onConfirm: async () => {
          try {
            ...
            setSnackbar({
              open: true,
              message: `✅ Import erfolgreich! ${normalizedParticipants.length} Kandidat*innen wurden importiert.`,
              severity: "success",
            });
          } catch (error) {
            ...
            setSnackbar({
              open: true,
              message: `❌ Fehler beim Import: ${error instanceof Error ? error.message : "Unbekannter Fehler"}`,
              severity: "error",
            });
          }
        },
  ...
  const importCompleteData = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setConfirmDialog({
        ...
        onConfirm: async () => {
          try {
            // Bereite Daten vor und normalisiere sie (ähnlich wie in jsonImport.ts)
            const normalizedData = {
              participants: data.participants,
              matchingNights: data.matchingNights?.map((night: any) => ({
                ...night,
                // Ensure date is present (backend requires it). Fallback to ausstrahlungsdatum or current date.
                date:
                  night.date ||
                  night.ausstrahlungsdatum ||
                  new Date().toISOString().split("T")[0],
              })),
              matchboxes: data.matchboxes?.map((box: any) => ({
                ...box,
              ...
              broadcastNotes: data.broadcastNotes || [],
            };
            ...
            setSnackbar({
              open: true,
              message:
                "✅ Kompletter Import erfolgreich! Seite wird neu geladen...",
              severity: "success",
            });
      ...
      setSnackbar({
        open: true,
        message: `❌ Fehler beim Komplettimport: ${error instanceof Error ? error.message : "Unbekannter Fehler"}\n\nBitte überprüfen Sie die Backup-Datei.`,
        severity: "error",
      });
    } finally {
      setIsLoading(false);
  ...
  const soldMatchboxes = matchboxes.filter(
    (mb) => mb.matchType === "sold" && mb.price && typeof mb.price === "number",
  );
  const totalRevenue = soldMatchboxes.reduce(
    (sum, mb) => sum + (mb.price || 0),
    0,
  );
  ...
  const exportItems = [
    ...
    {
      title: "Matching Nights",
      count: matchingNights.length,
      onClick: exportMatchingNights,
      icon: <NightlifeIcon />,
      disabled: matchingNights.length === 0,
    },
    {
      title: "Matchboxes",
      count: matchboxes.length,
      onClick: exportMatchboxes,
      icon: <InventoryIcon />,
      disabled: matchboxes.length === 0,
    },
    {
      title: "Strafen/Transaktionen",
    ...
    <Box>
        ...
        <Card sx={{ mb: 4 }}>
          ...
          <CardContent>
            ...
            <Alert severity="info" sx={{ mt: 2 }}>
              <Typography variant="body2">
                <strong>💡 Hinweis:</strong> Die Startsumme wird lokal
                gespeichert und beeinflusst die Berechnung des aktuellen
                Kontostands basierend auf verkauften Matchboxes.
              </Typography>
            </Alert>
          </CardContent>
        </Card>
      ...
      {renderContext !== "json-import" && (
        <Card sx={{ mb: 4 }}>
          ...
          <CardContent>
            {penalties.length === 0 ? (
              <Alert severity="info">Noch keine Strafen erfasst</Alert>
            ) : (
              <Box
                sx={{
                  display: "grid",
                      ...
                      <CardContent>
                        ...
                        >
                            ...
                            <Box>
                              ...
                              <Box sx={{ display: "flex", gap: 1, mt: 1 }}>
                                ...
                                <Chip
                                  ...
                                  color={
                                    penalty.amount >= 0 ? "success" : "error"
                                  }
                                  size="small"
                                  icon={
                                    penalty.amount >= 0 ? (
                          ...
                          <Box>
                            <Tooltip title="Bearbeiten">
                              <IconButton
                                onClick={() => openEditPenaltyDialog(penalty)}
                                color="primary"
                              >
                                <EditIcon />
                              </IconButton>
                        ...
                        <Typography variant="caption" color="text.secondary">
                          Datum:{" "}
                          {new Date(penalty.date).toLocaleDateString("de-DE")} |
                          Erstellt:{" "}
                          {new Date(penalty.createdAt).toLocaleString("de-DE", {
                            day: "2-digit",
                            month: "2-digit",
                            year: "numeric",
                            hour: "2-digit",
            ...
            <Alert severity="warning" sx={{ mt: 3 }}>
              <Typography variant="body2">
                <strong>💡 Hinweis:</strong> Strafen werden automatisch vom
                aktuellen Kontostand abgezogen und sind sofort in der
      ...
      {renderContext === "json-import" && (
        <Card sx={{ mb: 4 }}>
          ...
          <CardContent>
            ...
            <Alert severity="info" sx={{ mt: 2 }}>
              <Typography variant="body2">
                <strong>JSON Import:</strong> Gender wird automatisch von w/m zu
                F/M konvertiert. Der Import ersetzt alle bestehenden
                Kandidat*innen.
              </Typography>
            </Alert>
          </CardContent>
        ...
        <Card sx={{ mb: 4 }}>
          ...
          <CardContent>
                ...
                >
                  <CardContent>
                    ...
                    <Button
                      ...
                      color={
                        !item.disabled && !isLoading ? "success" : "primary"
                      }
                    >
                      Exportieren
                    </Button>
            ...
            <Alert severity="success" sx={{ mt: 2 }}>
              <Typography variant="body2">
                <strong>💡 Tipp:</strong> Alle Exports werden als JSON-Dateien
                mit Datum heruntergeladen. Der Komplettexport enthält alle Daten
                in einer strukturierten Datei.
              </Typography>
            </Alert>
          </CardContent>
        </Card>
        ...
        <Card sx={{ border: "2px solid", borderColor: "error.main", mb: 4 }}>
          ...
          <CardContent sx={{ textAlign: "center" }}>
            ...
            <Typography variant="body1" color="error.main" sx={{ mb: 3 }}>
              ⚠️ Diese Aktion löscht ALLE Daten unwiderruflich ({totalEntries}{" "}
              Einträge)
            </Typography>
            <Button
              variant="contained"
          ...
          <CardContent>
            <Stack spacing={4}>
              ...
              <Box sx={{ textAlign: "center" }}>
                ...
                <Typography
                  variant="h5"
                  sx={{ fontWeight: 700, mb: 2, color: "info.main" }}
                >
                ...
                <Typography
                  ...
                  sx={{ mb: 3 }}
                >
                  🔄 Erkennt und korrigiert vertauschte Mann/Frau-Zuordnungen in
                  Matching Nights
                </Typography>
                <Button
                  variant="contained"
              ...
              <Box sx={{ textAlign: "center" }}>
                ...
                <Alert severity="info" sx={{ mb: 3, textAlign: "left" }}>
                  ...
                  <Typography variant="body2" component="div">
                    ...
                    • Matching Night: 21:00 Uhr (danach)
                    <br />
                    • Nur Uhrzeiten werden gesetzt
                    <br />• Daten bleiben komplett unverändert
                  </Typography>
                </Alert>
                <Button
                  variant="contained"
        ...
        <Card sx={{ border: "2px solid", borderColor: "warning.main", mb: 4 }}>
          ...
          <CardContent sx={{ textAlign: "center" }}>
            ...
            <Typography variant="body1" color="warning.main" sx={{ mb: 3 }}>
              🗑️ Löscht Cache, Cookies und alle gespeicherten Daten (Datenbank
              bleibt erhalten)
            </Typography>
            <Button
              variant="contained"
            ...
            <Alert severity="info">
              <Typography variant="body2" component="div">
                ...
                <Box component="ul" sx={{ mt: 1, pl: 2 }}>
                  <li>
                    <strong>Export:</strong> Alle Daten können einzeln oder
                    komplett als JSON exportiert werden
                  </li>
                  <li>
                    Alle Löschvorgänge sind <strong>unwiderruflich</strong>
                  </li>
                  <li>Vor jedem Vorgang erscheint eine Sicherheitsabfrage</li>
                  <li>Kompletter Reset erfordert doppelte Bestätigung</li>

                  <li>
                    <strong>JSON-Import:</strong> Gender wird automatisch von
                    w/m zu F/M konvertiert
                  </li>
      ...
      >
        ...
        <DialogContent>
          <Stack spacing={3} sx={{ mt: 2 }}>
            ...
            <TextField
              ...
              onChange={(e) =>
                setPenaltyForm({ ...penaltyForm, description: e.target.value })
              }
        ...
        <DialogActions>
          <Button onClick={closePenaltyDialog}>Abbrechen</Button>
          <Button
            onClick={savePenalty}
            variant="contained"
            startIcon={<SaveIcon />}
            disabled={
              ...
              !penaltyForm.reason ||
              !penaltyForm.amount ||
              isNaN(parseFloat(penaltyForm.amount)) ||
              parseFloat(penaltyForm.amount) === 0
            }
          >
      ...
      >
        ...
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
            Geben Sie die neue Startsumme für das Budget ein. Dies beeinflusst
            die Berechnung des aktuellen Kontostands.
          </Typography>
          <TextField
            autoFocus
            fullWidth
          ...
          <Alert severity="info">
            <Typography variant="body2">
              <strong>Aktuelle Startsumme:</strong>{" "}
              {budgetSettings.startingBudget.toLocaleString("de-DE")} €<br />
              <strong>Verkaufte Matchboxes:</strong>{" "}
              {totalRevenue.toLocaleString("de-DE")} €<br />
        ...
        <DialogActions>
          <Button onClick={closeBudgetDialog}>Abbrechen</Button>
          <Button
...
const AdminPanelMUI: React.FC = () => {
  ...
  const loadAllData = async () => {
    try {
      ...
      console.log("✅ Admin Panel: Daten erfolgreich geladen");
    } catch (error) {
      console.error("❌ Fehler beim Laden der Admin Panel-Daten:", error);
      setSnackbar({
        open: true,
        message:
          "Fehler beim Laden der Daten. Bitte prüfen Sie die Verbindung.",
        severity: "error",
      });
    }
  };
  ...
  const handleImportParticipantsJSON = async (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    const file = e.target.files?.[0];
    if (!file) return;
    ...
    try {
      ...
      const participantsArray = Array.isArray(arr)
        ? arr
        : arr.participants || [];
      ...
      // Daten normalisieren und Gender-Mapping durchführen
      const normalizedParticipants = participantsArray.map(
        (participant: any) => {
          // Gender-Mapping: w/m -> F/M
          let gender = participant.gender;
          if (gender === "w" || gender === "weiblich" || gender === "female") {
      ...
      try {
        ...
        await loadAllData();
        alert(
          `✅ Import erfolgreich! ${normalizedParticipants.length} Kandidat*innen wurden importiert.`,
        );
      } catch (error) {
        console.error("Fehler beim Import:", error);
        alert(
        ...
        <Card>
          ...
          {activeTab === "participants" && (
            <Box sx={{ p: 3 }}>
              ...
              {/* Action Buttons */}
              <Box
                sx={{ mt: 4, mb: 3, display: "flex", gap: 2, flexWrap: "wrap" }}
              >
                <Button
                  variant="contained"
                ...
                >
                  {editingParticipant
                    ...
                    : "Einzelne Kandidat*innen hinzufügen"}
                </Button>
