...
const JsonImportManagement: React.FC<JsonImportManagementProps> = ({
  onDataUpdate,
}) => {
  ...
  const loadDataCounts = async () => {
    try {
      // Lade Datenzählungen via API Services
      ...
      console.log("✅ JSON Import Management: Datenzählungen geladen");
    } catch (error) {
    ...
    try {
      ...
      if (success) {
        ...
        setSnackbar({
          open: true,
          message:
            "❌ Fehler beim Importieren der JSON-Daten. Bitte überprüfe die Browser-Konsole für Details.",
          severity: "error",
        });
      }
    } catch (error) {
  ...
  return (
    <Box>
      ...
      <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
        Importiere Daten aus JSON-Dateien für neue Versionen. Alle bestehenden
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          ...
          >
            <Typography variant="h6">Verfügbare JSON-Dateien</Typography>
          ...
          {availableFiles.length === 0 ? (
            <Alert severity="info">
              Keine JSON-Dateien gefunden. Stelle sicher, dass JSON-Dateien im
              /json/ Verzeichnis vorhanden sind.
            </Alert>
          ) : (
            <List>
                ...
                <React.Fragment key={file}>
                  <ListItem>
                    ...
                    <ListItemSecondaryAction>
                      <Button
                        variant={
                          selectedFile === file ? "contained" : "outlined"
                        }
                        size="small"
      ...
      <Box sx={{ display: "flex", gap: 2, flexWrap: "wrap" }}>
        <Button
          variant="contained"
          startIcon={
            isImporting ? <CircularProgress size={20} /> : <UploadIcon />
          }
      ...
      >
        ...
        <DialogContent>
          <Typography>
            Möchtest du die Daten aus <strong>{selectedFile}</strong>{" "}
            importieren?
          </Typography>
          <Alert severity="warning" sx={{ mt: 2 }}>
            <strong>Achtung:</strong> Alle bestehenden Daten werden dabei
            überschrieben!
          </Alert>
        </DialogContent>
        <DialogActions>
