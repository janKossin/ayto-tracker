...
const CoupleAvatars: React.FC<{
  ...
  const finalWomanPhoto =
    womanPhoto ||
    (womanParticipant?.photoUrl && womanParticipant.photoUrl.trim() !== ""
      ? womanParticipant.photoUrl
      : null);
  const finalManPhoto =
    manPhoto ||
    (manParticipant?.photoUrl && manParticipant.photoUrl.trim() !== ""
      ? manParticipant.photoUrl
      : null);
  // Tooltip content
  const tooltipLines = [
    `${womanName} & ${manName}`,
  ...
  const getBorderColor = () => {
    switch (matchType) {
      ...
      case "sold":
        return "info.main";
      default:
        return "grey.400"; // Default gray for unconfirmed matches
    }
  };
...
const ParticipantCard: React.FC<{
  ...
  const genderColor =
    participant.gender === "F" ? "secondary.main" : "primary.main";
    ...
    >
      <CardContent sx={{ p: 1, height: "100%" }}>
        ...
        >
          ...
          <Box
            onDrop={
              isPerfectMatch ? undefined : (e) => onDrop(e, pairIndex, "woman")
            }
            onDragOver={
              isPerfectMatch
                ? undefined
                : (e) => onDragOver(e, pairIndex, "woman")
            }
            ...
            sx={{
              ...
              borderColor:
                dragOverSlot === "woman" ? "primary.main" : "transparent",
              borderRadius: 2,
              ...
              >
                <Avatar
                  ...
                  sx={{
                    width: 40,
                    height: 40,
                    bgcolor: womanParticipant?.photoUrl
                      ? undefined
                      : womanParticipant?.gender === "F"
                        ? "secondary.main"
                        : "primary.main",
                    border: "1px solid",
                    borderColor:
                ...
                >
                  {!womanParticipant?.photoUrl &&
                    (womanParticipant?.name?.charAt(0) || "?")}
                </Avatar>
                <Typography
                  variant="caption"
                  sx={{
          ...
          <Box
            onDrop={
              isPerfectMatch ? undefined : (e) => onDrop(e, pairIndex, "man")
            }
            onDragOver={
              isPerfectMatch
                ? undefined
                : (e) => onDragOver(e, pairIndex, "man")
            }
            onDragLeave={isPerfectMatch ? undefined : onDragLeave}
            sx={{
              flex: 1,
              ...
              p: 0.5,
              border:
                dragOverSlot === "man" ? "2px dashed" : "2px solid transparent",
              borderColor:
                dragOverSlot === "man" ? "primary.main" : "transparent",
              borderRadius: 2,
              ...
              >
                <Avatar
                  ...
                  sx={{
                    width: 40,
                    height: 40,
                    bgcolor: manParticipant?.photoUrl
                      ? undefined
                      : manParticipant?.gender === "F"
                        ? "secondary.main"
                        : "primary.main",
                    border: "1px solid",
                    borderColor:
        ...
        <CardContent>
          ...
          >
            {matchingNight.pairs.map((pair, index) => {
              ...
              const isConfirmedPerfectMatch = isPairConfirmedAsPerfectMatch(
                pair,
                matchingNight,
                matchboxes,
              );
...
const calculateStatistics = (
  ...
  const currentBalance =
    startingBudget - totalRevenue - totalPenalties + totalCredits;

  // Get latest matching night lights
  const latestMatchingNight = matchingNights.sort((a, b) => {
  ...
  console.log("🔍 Frontend Lichter-Debug:", {
    ...
    currentLights,
    allMatchingNights: matchingNights.map((mn) => ({
      name: mn.name,
      lights: mn.totalLights,
      date: mn.ausstrahlungsdatum || mn.createdAt,
    })),
  });
...
const OverviewMUI: React.FC = () => {
  ...
  const [expandedMatchingNights, setExpandedMatchingNights] = useState<
    Set<number>
  >(new Set());
  ...
  const {
    ...
    triggerCalculation,
    clearCache: clearProbabilityCache,
  ...
  const loadAllData = async () => {
    try {
      ...
      console.log("🔍 API Response Received:", {
        ...
        matchingNightsCount: matchingNightsData.length,
        matchboxesCount: matchboxesData.length,
        penaltiesCount: penaltiesData.length,
      });
      ...
      console.log("✅ Frontend: Daten aus API geladen und State aktualisiert");
    } catch (error) {
      console.error("❌ Fehler beim Laden der Frontend-Daten aus API:", error);
    }
  };
  ...
  const [dragOverPairIndex, setDragOverPairIndex] = useState<number | null>(
    null,
  );
  const [dragOverSlot, setDragOverSlot] = useState<"woman" | "man" | null>(
    null,
  );
  const [placedParticipants, setPlacedParticipants] = useState<Set<string>>(
    new Set(),
  );
  ...
  const getAutoPlaceablePerfectMatches = () => {
    return getValidPerfectMatchesBeforeDateTime(matchboxes, new Date()).slice(
      0,
      10,
    ); // Max 10 Perfect Matches (one per container)
  };
  ...
  const [draggedParticipants, setDraggedParticipants] = useState<{
    woman?: Participant;
    man?: Participant;
  }>({});
  const [dragOverTarget, setDragOverTarget] = useState<"woman" | "man" | null>(
    null,
  );
  ...
  const women = participants.filter(
    (p) =>
      p.gender === "F" &&
      (p.status === "Aktiv" ||
        p.status === "aktiv" ||
        p.status === "Perfekt Match"),
  );
  const men = participants.filter(
    (p) =>
      p.gender === "M" &&
      (p.status === "Aktiv" ||
        p.status === "aktiv" ||
  ...
  const pairProbabilities: Record<
    string,
    Record<string, number>
  > = probabilityResult?.probabilityMatrix || {};

  // Helper: Check if participant has a perfect match (nur ausgestrahlte Matchboxes)
  const hasConfirmedPerfectMatch = (
    ...
    return matchboxes.some((mb) => {
      ...
      const isCorrectParticipant =
        gender === "F"
          ? mb.woman === participantName
          : mb.man === participantName;
  ...
  const isPairInPerfectLightMatchingNight = (
    womanName: string,
    manName: string,
  ): boolean => {
    const now = new Date();
      ...
      if (mn.ausstrahlungsdatum && mn.ausstrahlungszeit) {
        try {
          const broadcastDate = new Date(
            `${mn.ausstrahlungsdatum}T${mn.ausstrahlungszeit}`,
          );
  ...
  const isDefinitivelyExcluded = (
    ...
    const excludedByPerfectMatch =
      (womanHasPerfectMatch && womanPerfectPartner !== manName) ||
  ...
  const getAllMatchingNightsTogether = (
    womanName: string,
    manName: string,
  ): { nightNumbers: number[]; allLights: number[] } => {
    const sortedNights = [...matchingNights].sort((a, b) => {
      const dateA = a.ausstrahlungsdatum
        ? new Date(a.ausstrahlungsdatum)
        : new Date(a.createdAt);
      const dateB = b.ausstrahlungsdatum
        ? new Date(b.ausstrahlungsdatum)
    ...
    for (let i = 0; i < sortedNights.length; i++) {
      ...
      const pairExists = night.pairs.some(
        (pair) => pair.woman === womanName && pair.man === manName,
      );
      if (pairExists) {
    ...
    try {
      ...
      const perfectMatchLights = completePairs.filter((pair) =>
        matchboxes.some((mb) => {
          ...
          return (
            getMatchboxBroadcastDateTime(mb).getTime() <
            currentMatchingNightDate.getTime()
          );
        }),
      ).length;
      ...
      setSnackbar({
        open: true,
        message: `Matching Night "${autoGeneratedName}" mit allen 10 Paaren wurde erfolgreich erstellt!`,
        severity: "success",
      });
  ...
  const saveMatchbox = async () => {
    try {
      ...
      if (matchboxForm.matchType === "perfect") {
        const existingPerfectMatch = matchboxes.find(
          (mb) =>
            mb.matchType === "perfect" &&
        ...
        if (existingPerfectMatch) {
          setSnackbar({
            open: true,
            message: "Dieses Paar ist bereits als Perfect Match bestätigt!",
            severity: "error",
          });
          return;
        }
  ...
  const handleMatchingNightDragOver = (
    e: React.DragEvent,
    pairIndex: number,
    slot: "woman" | "man",
  ) => {
  ...
  const handleMatchingNightDrop = (
    ...
    if (
      placedParticipants.has(draggedParticipant.name || "") ||
      getAllConfirmedPerfectMatchParticipants().has(
    ...
    const targetPair = matchingNightForm.pairs[pairIndex] || {
      woman: "",
      man: "",
    };
    if (slot === "woman" && targetPair.man) {
      const manParticipant = participants.find(
        (p) => p.name === targetPair.man,
      );
      if (
        manParticipant &&
        manParticipant.gender === draggedParticipant.gender
      ) {
        setSnackbar({
          open: true,
    ...
    if (slot === "man" && targetPair.woman) {
      const womanParticipant = participants.find(
        (p) => p.name === targetPair.woman,
      );
      if (
        womanParticipant &&
        womanParticipant.gender === draggedParticipant.gender
      ) {
        setSnackbar({
          open: true,
  ...
  const removeParticipantFromPair = (
    pairIndex: number,
    slot: "woman" | "man",
  ) => {
  ...
  const handleDrop = (e: React.DragEvent, target: "woman" | "man") => {
    ...
    if (participantData) {
      ...
      if (
        getAllConfirmedPerfectMatchParticipants().has(participant.name || "")
      ) {
        setSnackbar({
          open: true,
          message: `${participant.name} ist bereits als Perfect Match bestätigt und kann nicht für eine neue Matchbox verwendet werden.`,
          severity: "error",
        });
        return;
      }
      if (
        (target === "woman" && participant.gender === "F") ||
  ...
  useEffect(() => {
    if (draggedParticipants.woman) {
      setMatchboxForm((prev) => ({
        ...prev,
        woman: draggedParticipants.woman?.name || "",
      }));
    }
    if (draggedParticipants.man) {
      setMatchboxForm((prev) => ({
        ...prev,
        man: draggedParticipants.man?.name || "",
      }));
    }
  }, [draggedParticipants]);
      ...
      >
        <Card sx={{ mb: 4 }}>
              ...
              >
                ...
                <Card
                  sx={{
                    width: 280,
                    background:
                      "linear-gradient(135deg, #BD0A16 0%, #CD9536 100%)",
                    color: "white",
                    boxShadow: 4,
                    borderRadius: 3,
                    transition: "all 0.3s ease",
                  ...
                  <CardHeader
                    ...
                    subheader={
                      <Typography variant="body2" sx={{ color: "white" }}>
                        Ziehe Kandidat*in hier hinein • Header klicken zum
                        Verschieben
                      </Typography>
                    }
                    sx={{
                    ...
                    >
                      ...
                      >
                        {matchboxForm.woman ? (
                          ...
                          >
                            {(() => {
                              const woman = women.find(
                                (w) => w.name === matchboxForm.woman,
                              );
                              const hasPhoto =
                                woman?.photoUrl && woman.photoUrl.trim() !== "";
                              return (
                                <Avatar
                      ...
                      >
                        {matchboxForm.man ? (
                          ...
                          >
                            {(() => {
                              const man = men.find(
                                (m) => m.name === matchboxForm.man,
                              );
                              const hasPhoto =
                                man?.photoUrl && man.photoUrl.trim() !== "";
                              return (
                                <Avatar
                    ...
                    >
                      {participants
                        ...
                        .map((participant) => {
                          ...
                          const isUnavailable =
                            isConfirmedPM ||
                            placedParticipants.has(participant.name || "");
                          return (
                            <ParticipantCard
                              key={participant.id}
                              participant={participant}
                  ...
                  .map((matchbox) => {
                    const additionalInfo = [
                      matchbox.matchType === "sold" && matchbox.price
                        ? `${matchbox.price.toLocaleString("de-DE")} €`
                        : null,
          ...
          <TabPanel value={activeTab} index={4}>
            ...
            {(() => {
              const matchboxesWithoutBroadcast = matchboxes.filter(
                (mb) => !mb.ausstrahlungsdatum || !mb.ausstrahlungszeit,
              );
              const matchingNightsWithoutBroadcast = matchingNights.filter(
                (mn) => !mn.ausstrahlungsdatum || !mn.ausstrahlungszeit,
              );
              ...
              if (
                ...
                return (
                  <Alert severity="warning" sx={{ mb: 3 }}>
                    ...
                    {matchboxesWithoutBroadcast.length > 0 && (
                      <div>
                        • {matchboxesWithoutBroadcast.length} Matchbox
                        {matchboxesWithoutBroadcast.length > 1 ? "en" : ""} ohne
                        Ausstrahlungsdatum/-zeit (werden nicht in der Matrix
                        berücksichtigt)
                      </div>
                    )}
                    {matchingNightsWithoutBroadcast.length > 0 && (
                      <div>
                        • {matchingNightsWithoutBroadcast.length} Matching Night
                        {matchingNightsWithoutBroadcast.length > 1
                          ? "s"
                          : ""}{" "}
                        ohne Ausstrahlungsdatum/-zeit (verwenden
                        Erstellungsdatum als Fallback)
                      </div>
                    )}
                    <div style={{ marginTop: "8px" }}>
                      → Bitte Ausstrahlungsdaten im{" "}
                      <strong>Admin-Panel → Ausstrahlungsplan</strong>{" "}
                      vervollständigen.
                    </div>
                  </Alert>
                );
            ...
            <Card sx={{ height: "fit-content", mb: 3 }}>
              ...
              {probabilityStatus.isCalculating && (
                <Box sx={{ mx: 2, mt: 2 }}>
                  ...
                  >
                    {probabilityStatus.progress}% -{" "}
                    {probabilityStatus.currentStep}
                  </Typography>
                </Box>
              )}
              ...
              {probabilityResult && !probabilityStatus.isCalculating && (
                <Alert severity="info" sx={{ mx: 2, mt: 2 }}>
                  ...
                  {probabilityResult.totalValidMatchings.toLocaleString()}{" "}
                  gültige Kombinationen gefunden in{" "}
                  {(probabilityResult.calculationTime / 1000).toFixed(2)}s
                  {probabilityResult.limitReached && " (Limit erreicht)"}
                  {probabilityResult.fixedPairs.length > 0 &&
                    ` • ${probabilityResult.fixedPairs.length} fixierte Paare`}
                </Alert>
              )}
              ...
              <CardContent sx={{ p: 0, "&:last-child": { pb: 0 } }}>
                  ...
                  !probabilityStatus.error && (
                    <Box sx={{ p: 3, textAlign: "center" }}>
                      <Typography
                        ...
                        sx={{ mb: 2 }}
                      >
                        Keine Daten vorhanden. Bitte zuerst Kandidat*innen und
                        Matching Nights hinzufügen.
                      </Typography>
                      <Button
                        variant="outlined"
                ...
                {probabilityStatus.error && (
                  <Box sx={{ p: 3, textAlign: "center" }}>
                    ...
                    <Box
                      component="ul"
                      sx={{ textAlign: "left", display: "inline-block", mb: 2 }}
                    >
                      ...
                      <Table size="small">
                        <TableHead>
                          <TableRow>
                            ...
                            ></TableCell>
                            {men.map((man) => {
                              const hasPerfectMatch = hasConfirmedPerfectMatch(
                                man.name!,
                                "M",
                              );
                              const partner = getPerfectMatchPartner(
                                man.name!,
                                "M",
                              );
                              return (
                                  ...
                                  >
                                    <Avatar
                                      ...
                                      sx={{
                                        ...
                                        fontSize: "0.9rem",
                                        bgcolor: "white",
                                        border: hasPerfectMatch
                                          ? "2px solid"
                                          : "1px solid",
                                        borderColor: hasPerfectMatch
                                          ? "success.dark"
                                          : "grey.300",
                                      }}
                                    >
                        ...
                        <TableBody>
                          {women.map((woman) => {
                            const womanHasPerfectMatch =
                              hasConfirmedPerfectMatch(woman.name!, "F");
                            const womanPartner = getPerfectMatchPartner(
                              woman.name!,
                              "F",
                            );
                            return (
                              <TableRow key={woman.id}>
                                  ...
                                  >
                                    ...
                                    >
                                      <Typography
                                        ...
                                        sx={{
                                          ...
                                          wordBreak: "break-word",
                                          color: "black",
                                          fontWeight: womanHasPerfectMatch
                                            ? "bold"
                                            : "normal",
                                        }}
                                      >
                                    ...
                                    <Avatar
                                      ...
                                      sx={{
                                        ...
                                        fontSize: "0.9rem",
                                        bgcolor: "white",
                                        border: womanHasPerfectMatch
                                          ? "2px solid"
                                          : "1px solid",
                                        borderColor: womanHasPerfectMatch
                                          ? "success.dark"
                                          : "grey.300",
                                      }}
                                    >
                                ...
                                {men.map((man) => {
                                  ...
                                  const isConfirmedPerfectMatch =
                                    percentage === 100 ||
                                    (hasConfirmedPerfectMatch(
                                      woman.name!,
                                      "F",
                                    ) &&
                                      hasConfirmedPerfectMatch(
                                      ...
                                      getPerfectMatchPartner(
                                        woman.name!,
                                        "F",
                                      ) === man.name!) ||
                                    isPairInPerfectLightMatchingNight(
                                      woman.name!,
                                      man.name!,
                                    );

                                  // Prüfe, ob es definitiv ausgeschlossen ist (durch Box-Entscheidungen)
                                  const isDefinitelyExcluded =
                                    isDefinitivelyExcluded(
                                      woman.name!,
                                      man.name!,
                                    );
                                  ...
                                  const allMatchingNightsTogether =
                                    getAllMatchingNightsTogether(
                                      woman.name!,
                                      man.name!,
                                    );
                                  ...
                                  return (
                                    <TableCell
                                      ...
                                      sx={{
                                        ...
                                        color: isConfirmedPerfectMatch
                                          ? "success.main" // Perfect Match = Grün
                                          : isDefinitelyExcluded
                                            ? "error.main" // Definitiv ausgeschlossen = Rot
                                      ...
                                      title={`${woman.name} & ${man.name}: ${isConfirmedPerfectMatch ? "100" : percentage}%${
                                        isConfirmedPerfectMatch
                                          ...
                                          : isDefinitelyExcluded
                                            ...
                                            : allMatchingNightsTogether
                                                  .nightNumbers.length > 0
                                              ? ` (Matching Nights: ${allMatchingNightsTogether.nightNumbers.map((num: number, idx: number) => `#${num} (${allMatchingNightsTogether.allLights[idx]} Lichter)`).join(", ")})`
                                              : ""
                                      }`}
                                    >
                                      {/* Oberer Teil: Symbol */}
                                      ...
                                      {allMatchingNightsTogether.nightNumbers
                                        .length > 0 && (
                                        <Box
                                          sx={{
                                            display: "flex",
        ...
        <Dialog
          open={matchingNightDialog}
          onClose={() => setMatchingNightDialog(false)}
          maxWidth="lg"
          fullWidth
        >
          <DialogTitle>
              ...
              {false && !isMobile && (
                ...
                >
                  ...
                  <Typography
                    ...
                    sx={{ fontWeight: "bold" }}
                  >
                    {getAutoPlaceablePerfectMatches().length} bestätigte Perfect
                    Matches verfügbar
                  </Typography>
                </Box>
              )}
                ...
                >
                  <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                    ...
                    <TextField
                      ...
                      inputProps={{
                        min: (() => {
                          const perfectMatchLights =
                            matchingNightForm.pairs.filter(
                              (pair) =>
                                pair &&
                                pair.woman &&
                                pair.man &&
                  ...
                  >
                    ...
                    >
                      {Array.from({ length: 5 }, (_, index) => {
                        const pair = matchingNightForm.pairs[index] || {
                          woman: "",
                          man: "",
                        };
                        const isPerfectMatch = matchboxes.some(
                          (mb) =>
                            mb.matchType === "perfect" &&
                            mb.woman === pair.woman &&
                            mb.man === pair.man,
                        ...
                        return (
                          <MatchingNightPairContainer
                            ...
                            dragOverSlot={
                              dragOverPairIndex === index ? dragOverSlot : null
                            }
                    ...
                    >
                      {Array.from({ length: 5 }, (_, index) => {
                        const pair = matchingNightForm.pairs[index + 5] || {
                          woman: "",
                          man: "",
                        };
                        const isPerfectMatch = matchboxes.some(
                    ...
                    {Array.from({ length: 10 }, (_, index) => {
                      ...
                      return (
                        <Card
                          ...
                          sx={{
                            p: 2,
                            bgcolor: isPerfectMatch ? "warning.50" : "grey.50",
                          }}
                        >
                          <Box
                            sx={{
                          ...
                          >
                            <FormControl fullWidth>
                              ...
                              >
                                ...
                                {women.map((woman) => {
                                  const isUsed = matchingNightForm.pairs.some(
                                    (p, i) =>
                                      i !== index && p.woman === woman.name,
                                  );
                                  return (
                                    <MenuItem
                                      key={woman.id}
                                      value={woman.name}
                                      disabled={isUsed || isPerfectMatch}
                                    >
                                      {woman.name}{" "}
                                      {isUsed && "(bereits verwendet)"}{" "}
                                      {isPerfectMatch &&
                                        "(Perfect Match - gesperrt)"}
                                    </MenuItem>
                                  );
                                })}
                              </Select>
        ...
        <Dialog
          open={matchboxDialog}
          onClose={() => setMatchboxDialog(false)}
          maxWidth="md"
          fullWidth
        >
          <DialogTitle>
            <Box sx={{ display: "flex", flexDirection: "column", gap: 1 }}>
              ...
              <Typography variant="body2" color="text.secondary">
                {isMobile
                  ? "📱 Wähle Kandidat*innen aus den Listen aus"
                  : "💡 Ziehe Kandidat*innen aus der Übersicht hier hinein"}
              </Typography>
            </Box>
          </DialogTitle>
          <DialogContent>
              ...
              {!isMobile && (
                ...
                >
                  ...
                  <Box
                    ...
                    sx={{
                      border: "2px dashed",
                      borderColor: matchboxForm.woman
                        ? "secondary.main"
                        : "grey.300",
                      borderRadius: 2,
                      p: 3,
